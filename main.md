...
조직에 올바른 목적을 세우고, 구성원의 목적을 조직의 목적과 일치시키는 기법으로 OKR이 있다[2].

> [1] 매니지먼트, 피터 드러커, 2001
> [2] OKR은 평가 기법이 아니며, 목적 관리 기법임에 유의하라.
...
떠나가는 사람은 이전까지 하지 못했던 말을 보다 홀가분하게 할 수 있다. 가능하다면 부검 면담을 가진다. 부검 면담을 남은 사람들을 위한 자료로 사용한다.

다음은 넷플릭스가 사용하는 부검 템플릿이다:

- 왜 떠나는가
- 회사에서 배운 것
- 회사에 아쉬운 점
- 앞으로의 계획
- 넷플릭스의 메시지(리더의 메시지로 대체할 수 있다)
...
저성과자에게 통지 및 개선 방안 논의를 수행하였고, 성과 개선 기회(평가를 위한 개선 기회임을 인지시켜야 한다)를 부여하였고, 직무나 팀 재배치를 시도하는 등 다양한 방법을 시도하였지만 개선이 이루어지지 않았다면 최종적인 조치를 취해야 한다. 모든 시도를 하였음에도 다음과 같은 자기 합리화를 하지 말자:
...
해고를 하게 되었다면 감정적이지 않을 수 있도록 최대한 노력하라. 해고는 감정적으로 쉽지 않은 일이기 때문에 감정에 쉽게 휘둘릴 수 있다. 결정하였다면 번복하지 말고, 빠르게 결정사항을 이행하라.
